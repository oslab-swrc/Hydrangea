...
#if 0
/**
 * (CAUTION) Enclave restrictions:
 *  If you call file I/O operation in enclave, its operation aborted from system.
 */

/* (deprecated)
 * func: read_syscall_ctrl
 * desc: read syscall control flag 
         (exception OE_ENCLAVE_ABORTING: not permit file access in enclave)
 * returns: (void)
 */
static int read_syscall_ctrl() {
    int fd, ret;
    char buf[10];

    fd = lkl_sys_open("/var/lib/sgxlkl/.enable", O_RDONLY, 0);
    if (fd < 0)
    {
        lkl_printf("[ ETRI_LOG ] can't access '/var/lib/sgxlkl/.enable' file\n");
        return -1;
    }
    memset(buf, 0x00, 10);
    ret = lkl_sys_read(fd, buf, 10);
    if(ret < 0) {
        lkl_printf("[ ETRI_LOG ] can't read '/var/lib/sgxlkl/.enable' file\n");
        return -1;
    }
    buf[ret] = '\0';
    ...
    if (ret != 0)
    {
        lkl_printf("[ ETRI_LOG ] can't close file descriptor '/var/lib/sgxlkl/.enable' file\n");
        return -1;
    } 
    return 0;
}
...
// enable system call control
static int enable_syscall_ctrl = -1;

// system call stat report print period
// < 0 or "not defined env": do not print
// positive number : print period
static  int syscall_stat_report = -1;


/** func: is_syscall_ctrl_enabled
  * desc: If syscall ctrl enabled, return 1(otherwise 0)
  * returns: (int) 1; enabled, 0; disabled
  */
int is_syscall_ctrl_enabled() {
    return enable_syscall_ctrl;
}


/** func: get_syscall_stat_report_config
  * desc: If syscall stat report is activated, returns positive number(otherwise <= 0)
  * returns: (int) positive number; enabled, <= 0; disabled
  */
int get_syscall_stat_report_config() {
    return syscall_stat_report;
}
...
/** func: abort_sgxlkl
  ...
  * returns: (void)
  */
void exit_sgxlkl(char *message) {
    lkl_printf("%s\n", message);
    exit(1);
}

/** function: init_syscall_ctrl
  * desc: initialize whitelist based system call control functionality
  * arg0: (void)
  * return: (void)
  */
static void init_syscall_ctrl(char** envp) {    
    char* val = NULL;
    char* syscalls = NULL;
    char** env = NULL;
    int keylen = 0;
    int envlen = 0;
    int syscall_report = -1;

    // lkl_printf("[ ETRI_LOG ] ------ init_syscall_ctrl\n");
    // check CTRL_SYSCALL is exist, and value
    // lkl_printf("[ ETRI_LOG ] INFO: dispatch CTRL_SYSCALL.\n");
    for (env = envp; *env; env++) {
        envlen = strlen(*env);
        keylen = strlen(CTRL_SYSCALL);

        if(envlen >= keylen) {
            // Env key: CTRL_SYSCALL
            if(strncmp(CTRL_SYSCALL, *env, keylen) == 0) {
                val = (char *)(*env + keylen);
                // lkl_printf("%s\n", val);
                if(strncmp(val, "1", 1) == 0) {
                    enable_syscall_ctrl = 1;
                    lkl_printf("[ ETRI_LOG:INFO ] Set syscall control option: [ ENABLED ]\n");
                }
                else if(strncmp(val, "0", 1) == 0) {
                    enable_syscall_ctrl = 0;
                    lkl_printf("[ ETRI_LOG:INFO ] Set syscall control option: [ DISABLED ]\n");
                }
                else {
                    enable_syscall_ctrl = -1;
                    lkl_printf("[ ETRI_LOG:ERROR ] Set syscall control option: [ FAIL ].\n");
                }
            }
        }
    }
    
    // check SYSCALL_STAT is exist
    for (env = envp; *env; env++) {
        envlen = strlen(*env);
        keylen = strlen(SYSCALL_STAT);

        if(envlen >= keylen) {
            // SYSCALL_STAT
            if(strncmp(SYSCALL_STAT, *env, keylen) == 0) {
                val = (char *)(*env + keylen);
                if(strlen(val) > 0) {
                    syscall_report = atoi(val);
                }                
            }
        }
    }
    syscall_stat_report = syscall_report;

    // set system call whitelist
    // lkl_printf("[ ETRI_LOG ] INFO: dispatch ALLOW_SYSCALLS.\n");
    for (env=envp; *env; env++) {
        envlen = strlen(*env);
        keylen = strlen(ALLOW_SYSCALLS);

        if(envlen >= keylen) {                
            // ALLOW_SYSCALLS
            if(strncmp(ALLOW_SYSCALLS, *env, keylen) == 0) {
                val = (char *)(*env + keylen);                    
                syscalls = strdup(val);
                // lkl_printf("%s\n", syscalls);
            }
    ...
    set_allowable_syscalls(syscalls);
    lkl_printf("[ ETRI_LOG ] INFO: Syscall whitelist installed: [SUCCESS]\n");
    return;
}
...
void lkl_start_init()
{
    ...
    #ifdef ETRI_SYSCALL_CTRL /* Author: hjk, classact(c) */    
    const char **env = shm->env;
    // Initialize system call control functionality  
    init_syscall_ctrl(env);
    #endif
